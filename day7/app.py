...
with open('input.txt') as _file:
    ...
    tally = 0
    for line in _file:
        rule = line.replace("bags", "").replace("bag", "").replace(
            ".", "").rstrip().split("  contain ")
        rules = rule[1].split(", ")
        rule_dict = {}
        for color_rule in rules:
            color = ''.join(
                letter for letter in color_rule if letter.isalpha() or letter == " ").strip()
            number = int(''.join(num for num in color_rule if num.isdigit())
                         ) if 'no' not in color_rule else 0
